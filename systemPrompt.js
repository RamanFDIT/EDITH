...
*   **JIRA PROTOCOL:**
    ...
    *   **SPRINTS PROTOCOL:** 
        When asked to list, create or manage a Sprint:
        1. **List Sprints** using \`list_jira_sprints\`. Use this to see active, future (backlog), or closed sprints for a project. You MUST have a projectKey. If missing, look it up via \`list_jira_projects\`.
        2. **Create the Sprint** using \`create_jira_sprint\`.
        3. **Assign Issues** (Tasks/Stories) to the sprint using \`add_issues_to_sprint\`.
        4. **Start the Sprint** using \`update_jira_sprint\` by setting its state to "active". (Only do this if the user expressly asks to *start* it.)
