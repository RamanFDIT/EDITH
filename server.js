  ...
  try {
    ...
    async function processTTSQueue() {
        ...
        while (ttsQueue.length > 0) {
            // Peek at the first item, don't remove until done to maintain order
            const currentItem = ttsQueue[0];
                ...
                if (typeof audioResult === 'string') {
                    try {
                        ...
                        if (parsed.fallback === 'web-speech-api') {
                            ...
                            ttsQueue.shift(); // Remove from queue
                            continue;
                        }
                    } catch (e) { /* not JSON */ }
            ...
            // Remove the processed item and loop to the next
            ttsQueue.shift();
        }
    ...
    function queueAudioChunk(text) {
        ttsQueue.push({ text, seq: ttsSequence++ });
        processTTSQueue(); // Fire the processor loop
    }
    // --------------------------
        ...
        if (eventType === "on_chat_model_stream") {
            ...
            if (content) {
                ...
                if (/[.?!]\s$/.test(sentenceBuffer) && sentenceBuffer.length > 5) {
                    // Queue the chunk instead of firing immediately
    ...
    // Wait for the TTS queue to drain before sending 'done'
    while(ttsQueue.length > 0 || isProcessingTTS) {
  ...
  try {
    ...
    async function processTTSQueue() {
        ...
        while (ttsQueue.length > 0) {
            ...
            } catch (e) { console.error('[Voice] TTS Queue Error:', e); }
            ttsQueue.shift();
        }
        isProcessingTTS = false;
    }
    ...
    for await (const event of stream) {
      ...
      if (eventType === 'on_chat_model_stream') {
        ...
        if (content) {
          ...
          sentenceBuffer += content;
          if (/[.?!]\s$/.test(sentenceBuffer) && sentenceBuffer.length > 5) {
            queueAudioChunk(sentenceBuffer.trim()); // queue instead of fire-and-forget
            sentenceBuffer = '';
          }
        }
      } else if (eventType === 'on_tool_start') {
